...
NEP_Missip_plot <- NEP_plotter(NEP_Missip, legend_pos = "top", 
                               breaks = c(-3000, -1500, 0, 1500)) + labs(tag = "A", y = NULL)
NEP_Sacr_plot <- NEP_plotter(NEP_Sacr, legend_pos = "top") + labs(tag = "B", y = NULL)
NEP_Park_plot <- NEP_plotter(NEP_Park, legend_pos = "top") + labs(tag = "C", y = NULL)

NEPgrob <- arrangeGrob(NEP_Missip_plot, NEP_Sacr_plot, NEP_Park_plot, 
                       ncol = 3, nrow = 1, 
                       left = textGrob(expression(paste("Cumulative", ~O[2]~m^-2~d^-1, "(g)")),
...
```{r discharge_plotting, fig.height=3, fig.width = 4, echo = FALSE, warning = FALSE, fig.cap="Stream discharge (Q) in the Mississippi River, Park River, and Sacramento River during 2017. \\label{discharge_plotting}"}
ggplot() +
  ...
  scale_color_manual(values = c("#f1a340", "#998ec3", "#a1d76a")) +
...
```{r PAR_Q, echo = FALSE, warning = FALSE, message=FALSE, fig.width=6, fig.height=4, fig.cap="Relationship of discharge (Q) to gross primary production (GPP) (A, B, C) and ecosystem respiration (ER) (D, E, F) in the Mississippi (A, D), Sacramento (B, E), and Park River (C, F) during 2017. Each point represents a daily average.\\label{PAR_Q}"}
...
PAR_Q <- merge(Q, model_Mississippi, by = "date")
PAR_Q <- data.frame(date = PAR_Q$date, Q.Missip = PAR_Q$Q, 
                    GPP.Missip = PAR_Q$GPP, ER.Missip = PAR_Q$ER)
...
ylabel_ER <- textGrob(expression(paste("ER (g ", ~O[2]~m^-2~d^-1, ")")),
                      rot = 90, gp = gpar(cex = 0.9))
xlabel <- textGrob(expression(paste("Q (", m^3, " ", s^-1, ")")), gp = gpar(cex = 0.9))
...
```{r mississippi_plotting, fig.height=3, fig.width = 8, echo = FALSE, warning = FALSE, fig.cap="An annual estimate of GPP and ER in the Mississippi River, 2017. \\label{mississippi_plotting}"}
metabolismPlot(model_Mississippi, 
               filename = "./FinalProject/figures/MississippiR_MetabolismModel.png")
```

```{r sacramento_plotting, fig.height=3, fig.width = 8, echo = FALSE, warning = FALSE, fig.cap="An annual estimate of GPP and ER in the Sacramento River, 2017. \\label{sacr_plotting}"}
metabolismPlot(model_Sacramento, 
               filename = "./FinalProject/figures/SacramentoR_MetabolismModel.png")
```

```{r connecti_plotting, fig.height=3, fig.width = 8, echo = FALSE, warning = FALSE, fig.cap="An annual estimate of GPP and ER in the North Branch of the Park River, 2017. \\label{connecti_plotting}"}
