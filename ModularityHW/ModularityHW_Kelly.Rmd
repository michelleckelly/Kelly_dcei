...
# to improve effciency: trying vectorized functions
str(precip)
# idea: seperate data into location data and preciptiation dataframes
# location data
precip_metadata <- unique(data.frame(State = precip$state,
                                     StationName = precip$name,
                                     Lat = precip$lat,
                                     Lon = precip$lon))
# precipitation data
precip <- data.frame(StationName = precip$name,
                     Year = precip$year,
                     Precip = precip$data)
# subset to only non-NA precipitation data
precip <- subset(precip, !is.na(precip$Precip))
# determine which stations have x or more years of data

#aggregate(precip,
#          list(State = precip$state,
#               StationName = precip$name,
#               Lat = precip$lat,
#               Lon = precip$lat,
#               Year = precip$year,
#               Data = nrow(precip$data) > 40),
#          by = name, FUN = mean)
