...
# Introduction  
  ...
  Average surface temperature on Earth has increased by 0.7&deg;C since the onset of the industrial era [@UnitedNationsFrameworkConventiononClimateChange2007]. Widespread fossil fuel use, and the following spike in atmospheric CO2, have acted as a global insulation blanket: trapping heat in the atmosphere and slowing it's release [@Asfaw2017]. Increased temperatures are also impacting precipitation regimes; in the United States, increases in the severity of drought and flood events are expected [@Dore2005]. However, the impact of global change on a local level is expected to be more spatially heterogenous, with different regions of the US experiencing varied climate and precipitation responses [@Trenberth2011].  
...
# Methods  
  ...
  Yearly summary statistics were calculated by taking the mean observation within a state during any given year. To compare changes across time, a mean value for the duration of observations within a state was calculated, and subtracted from the annual value \ref{eq-dev}. 
\begin{equation}
\label{eq-dev}
...
We see that deviations in temperature from local averages have generally increased from 1950-2008 (p-value < 0), with rapid increases from 1980 onwards \ref{fig:mapping}. However, warming has not been homogenous across the USA: states at northern latitudes and those located in the American Southwest have experienced more extreme effects.

```{r mapping, echo = FALSE, fig.width = 11, fig.height = 5, fig.cap="\\label{fig:mapping}Change in temperature per decade, relative to 1950-2008 state average."}
...
We know that the Earth has undergone two main periods of warming within the last century, one from 1910 and 1945, and the other from 1976 to present day [@Walther2002]. These results support the latter half of that assessment. It is also expected that the 

```{r heatmap_precip, echo=FALSE, fig.width = 15, fig.height = 5, fig.cap="\\label{fig:heatmap_precip}Anomalies in annual precipitation, relative to 1950-2008 state average"}
...
Precipitation dynamics have changed from `r min(temp$year)` to `r max(temp$year)` (p-value = `r precip_lm_pval`). Visually, we can see that there are both intense "high" and "low" years (that is, a potential increase in flood and drought events) \ref{fig:heatmap_precip}. To further investigate this phenomenon, we could use United States Geological Survey (USGS) river discharge data as an additional flood and drought predictor. It is not clear by this analysis alone whether location is a predictor of precipitation change. A finer scale (county level) analysis may be useful.

We have determined that climate warming from `r min(temp$year)` to `r max(temp$year)` is highly visually apparent. Generally, the southwest and upper midwest regions are experiencing the most warming, while the southern and southwest regions experienced colder than average temperatures during 1960-1980.  

# References

# Supplemental

```{r barplot_temp, echo = FALSE, fig.width = 17, fig.height = 17, fig.cap = "\\label{fig:barplot_temp}Anomalies in annual mean temperature, relative to 1950-2008 state average"}
...
```{r heatmap_temp, echo=FALSE, include = FALSE, fig.width = 12, fig.height = 5, fig.cap="\\label{fig:heatmap_temp}Anomalies in annual mean temperature, relative to 1950-2008 state average"}
